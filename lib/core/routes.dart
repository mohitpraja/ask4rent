class Routes{
  static String splash='/splash_view';
  static String login='/login_view';
  static String dashboard='/dashboard_view';
  static String signup='/signup_view';
  static String otp='/otp_view';
  static String forgot='/forgot_view';
  static String property='/property_page';
  static String ownerSeeker='/owner_seeker_view';
  static String editProfile='/editprofile_view';
  static String profile='/profile_view';
  static String forgotPass='/forgotPass_view';
  static String adminHome='/admin_home_view';
  static String propertyDetail='/property_detail_view';
  static String savedProperty='/saved_property_view';
  static String addressMap='/address_map_view';
  static String addExecutive='/add_executive_view';
  static String addProperty='/add_property_view';
  static String executiveHome='/executive_home_view';
}